...
async function checkAddressBalance(env, chatId, addr) {
  ...
  try {
    if (!isValidAddress(addr)) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `❌ Invalid address: <code>${addr}</code>`);
    ...
    await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `📊 <b>Address:</b>\n<code>${addr}</code>\n\n💰 <b>Balance:</b> ${formatBtc(bal)} BTC (${bal} sats)\n📦 <b>UTXOs:</b> ${utxos.length}`);
  } catch (e) { await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `❌ Error checking balance: ${e.message}`); }
}
...
async function checkAddressTxs(env, chatId, addr) {
  ...
  } catch (e) { await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `❌ Error checking transactions: ${e.message}`); }
}
...
async function checkAllBalances(env, chatId) {
  ...
  try {
    ...
    if (!wallets.length) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '📭 No wallets imported.');
    await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `📊 Checking ${wallets.length} wallet(s)...`);
    let msg = '📊 <b>Wallet Balances:</b>\n\n', grand = 0;
    for (const w of wallets) {
      const addrs = getAllAddresses(w.mnemonic || w.wif || w.hex || w.xprv, parseInt(SCAN_DEPTH) || 5);
...
async function sweepAll(env, chatId, targetAddr = null) {
  ...
  if (!cid || !dest) return;
  if ((await env.WALLETS.get('PAUSED')) === 'true') return await sendMsg(TELEGRAM_BOT_TOKEN, cid, '⏸️ Paused.');

  try {
    const wallets = await getWallets(env, cid);
    if (!wallets.length) return await sendMsg(TELEGRAM_BOT_TOKEN, cid, '❌ No wallets.');
    let report = [], total = 0;
    for (const w of wallets) {
...
async function recoverSeed(env, chatId, pattern, dictStr) {
  ...
  if (!wilds.length) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '❌ Use ? for unknown words.');
  if (wilds.length > 2 || dict.length > MAX_RECOVERY_ATTEMPTS) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `⚠️ Max 2 wildcards, ${MAX_RECOVERY_ATTEMPTS} words.`);
...
async function handleUpdate(update, env, ctx) {
  try {
    ...
    if (cmd === '/txs' || cmd === '/scan') { if (args.length > 0) return await checkAddressTxs(env, chatId, args[0]); return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `Usage: ${cmd} <bitcoin_address>`); }
    
    if (cmd === '/recover') {
      if (args.length < 2) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, 'Usage: /recover <pattern with ?> <word1,word2>');
      const dict = args.pop();
      const pattern = args.join(' ');
      if (ctx) ctx.waitUntil(recoverSeed(env, chatId, pattern, dict));
    ...
    if (cmd === '/add') {
      const a = args[0]; if (!a || !isValidAddress(a)) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '❌ Invalid. /add <addr>');
      const r = await getRecipients(env, chatId); if (r.includes(a)) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '⚠️ Exists.');
      r.push(a); await saveRecipients(env, chatId, r); return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `✅ Added <code>${a}</code>`);
    }
    if (cmd === '/remove') {
      ...
      r.splice(i, 1); await saveRecipients(env, chatId, r); return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `🗑️ Removed <code>${a}</code>`);
    }
    if (cmd === '/send') {
      const a = args[0]; if (!a || !isValidAddress(a)) return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '❌ /send <addr>');
      await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `📤 Sweeping to <code>${a}</code>...`); return await sweepAll(env, chatId, a);
    }

    if (t.includes(',')) return await importBulk(env, chatId, t);

    const type = detectKeyType(t);
    if (['mnemonic', 'wif', 'hex', 'xprv'].includes(type)) { const r = await importSingleKey(env, chatId, t); return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, r.msg); }
    if (type === 'address') { const r = await getRecipients(env, chatId); if (!r.includes(t)) { r.push(t); await saveRecipients(env, chatId, r); return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, `✅ Added <code>${t}</code>`); } return await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '⚠️ Exists.'); }

    await sendMsg(TELEGRAM_BOT_TOKEN, chatId, '❓ Use /start', mainMenu());

  } catch (e) {
    console.error("Fatal error in handleUpdate:", e);
